...
// Theme Toggle Logic
const themeToggleArr = utils.$('#theme-toggle');
if (themeToggleArr && themeToggleArr.length > 0) {
  const $themeToggle = themeToggleArr[0];
...
// Share Button Logic
const shareBtnArr = utils.$('#share-btn');
if (shareBtnArr && shareBtnArr.length > 0) {
  const $shareBtn = shareBtnArr[0];
  $shareBtn.addEventListener('click', async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: '3D Periodic Table',
          text: 'Explore the elements in 3D!',
